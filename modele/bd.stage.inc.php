...
function getStage() {
    ...
    try {
        $cnx = connexionPDO();
        $req = $cnx->prepare("select * from stage,creneau WHERE STA_CRE=CRE_ID AND CRE_VALIDE=1 order by STA_FORM ,STA_CRE ");
        $req->execute();
...
function getStageByForma($STA_FORM) {
    $resultat = array();

    try {
        $cnx = connexionPDO();
        $req = $cnx->prepare("select * from stage,formation, creneau,enseignant,matiere where STA_ENS = ENS_ID and STA_FORM = FORM_CODE AND STA_CRE=CRE_ID and STA_MAT=MAT_CODE and STA_FORM =:formation ");
        $req->bindValue(':formation', $STA_FORM, PDO::PARAM_INT);
        $req->execute();
